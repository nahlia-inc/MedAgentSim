...
TRUST_LEVELS_5 = ["vlow", "low", "neutral", "high", "vhigh"]
...
class Persona:
  ...
  def infer_patient_trust_5level(self, personas, verbose=False):
    """Infer patient trust in this persona (doctor) as a 5-level belief.

    v0: purely LLM-rated, based on recent dialogue text.
    Stores no global state; intended to be stored on self.scratch.
    """
    # Only run for the doctor persona (simple first cut).
    if "doctor" not in self.name.lower():
      return None

    # Identify patient persona.
    patient = None
    for n, p in personas.items():
      if n != self.name and "patient" in n.lower():
        patient = p
        break
    if patient is None:
      # Fall back to any other persona.
      for n, p in personas.items():
        if n != self.name:
          patient = p
          break
    ...
    if patient is None:
      return {
        "levels": TRUST_LEVELS_5,
        "probs": [0.05, 0.15, 0.60, 0.15, 0.05],
        "confidence": 0.0,
        "evidence": [],
        "updated_at": None,
      }

    # Build a minimal conversation string.
    def _as_text(x):
      if x is None:
        return ""
      ...
      if isinstance(x, str):
        return x
      # Common: list of [speaker, utt] rows.
    ...
    doc_chat = _as_text(getattr(self.scratch, "chat", None))
    pat_chat = _as_text(getattr(patient.scratch, "chat", None))

    conversation_str = ""
    # Prefer a shared 'chat' if present, otherwise combine.
    if doc_chat and pat_chat:
      conversation_str = (doc_chat + "\n" + pat_chat).strip()
    elif doc_chat:
      conversation_str = doc_chat.strip()
    elif pat_chat:
      conversation_str = pat_chat.strip()

    # If still empty, fall back to action descriptions (better than nothing).
    if not conversation_str:
      conversation_str = (
        f"{self.name}: {getattr(self.scratch, 'act_description', '')}\n"
        f"{patient.name}: {getattr(patient.scratch, 'act_description', '')}"
      ).strip()

    out = rate_trust_5level_llm(self, patient, conversation_str, verbose=verbose)
    # Attach metadata
    out["updated_at"] = self.scratch.curr_time.isoformat() if getattr(self.scratch, "curr_time", None) else None
    return out
  ...
  def move(self, maze, personas, curr_tile, curr_time):
    ...
    # --- Trust layer v0 ---
    # Doctor estimates patient's trust as a 5-level belief; used to bias next action.
    try:
      trust_belief = self.infer_patient_trust_5level(personas, verbose=False)
      if trust_belief is not None:
        self.scratch.patient_trust_belief = trust_belief
    except Exception:
      # Keep prior/default belief if inference fails.
      pass
